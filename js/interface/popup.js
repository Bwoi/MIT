...
port.onMessage.addListener(function(answer) {
	if (answer.method === 'hundred') {
		if (answer.message === 'ended') {
			...
			const undefinedEl = document.getElementById('undefined');
...
document.addEventListener('DOMContentLoaded', function(){
	...
	startProcess.addEventListener('click', function(){
		...
		const completedCards = document.getElementById('completed');
		completedCards.innerHTML = 0;


		const splited = allCodes.value.split(', ');
		const el = document.getElementById('amount');
		console.log(el);
		el.innerHTML = splited[0] !== '' ? splited.length : 0;
		const descript = descriptionStyle.value !== '' ? '<span style="' + descriptionStyle.value + '">' + description.value + '</span>' : description.value;
		setTimeout(function(){
			port.postMessage({
				method: 'hundred',
